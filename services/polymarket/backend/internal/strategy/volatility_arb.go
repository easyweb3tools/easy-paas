...
// and then applies a contrarian pull-to-0.5 opportunity on the affected market.
type VolatilityArbStrategy struct {
...
func (s *VolatilityArbStrategy) Name() string { return "volatility_arb" }

func (s *VolatilityArbStrategy) RequiredSignals() []string { return []string{"volatility_spread"} }

func (s *VolatilityArbStrategy) DefaultParams() json.RawMessage {
	return json.RawMessage(`{"min_edge_pct":0.04,"yes_extreme_min":0.72,"yes_extreme_max":0.28,"mean_revert_weight":0.55}`)
}

func (s *VolatilityArbStrategy) SetParams(raw json.RawMessage) error {
	var p struct {
...
func (s *VolatilityArbStrategy) Evaluate(ctx context.Context, signals []models.Signal) ([]models.Opportunity, error) {
	...
	reasoning := fmt.Sprintf("volatility_arb market=%s side=%s yes_ask=%s p_yes_expected=%.2f entry=%s",
