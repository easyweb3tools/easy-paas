...
export default function SettingsPage() {
  ...
  const legacyTokenAuth = useMemo(() => process.env.NEXT_PUBLIC_LEGACY_TOKEN_AUTH === "1", []);
      ...
      <section className="glass-panel rounded-xl border border-[color:var(--border)] bg-[var(--surface)] shadow-[var(--shadow)]">
        <div className="border-b border-[color:var(--border)] px-6 py-4">
          ...
          <p className="text-xs text-[var(--muted)]">
            UI requests default to cookie-based auth (httpOnly cookie, browser auto-sent with requests).
          </p>
        </div>
        <div className="grid gap-3 px-6 py-5 text-sm">
          <div className="rounded-xl border border-[color:var(--border)] bg-[var(--surface-strong)] p-4">
            <p className="text-xs text-[var(--muted)]">
              Recommended: sign in via platform auth endpoint and let backend set secure cookie.
            </p>
            {legacyTokenAuth ? (
              <>
                <p className="mt-3 text-xs font-semibold text-[var(--muted)]">Legacy Bearer token (localStorage compatibility mode)</p>
                <textarea
